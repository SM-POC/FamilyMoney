...
export const OutgoingsHubView: React.FC<OutgoingsHubViewProps> = ({ profile, setProfile, totalSubSpend }) => {
  const [newBill, setNewBill] = useState({ description: '', amount: '', category: 'Utilities', isSubscription: false, contractEndDate: '' });
  ...
  const handleAddBill = (e: React.FormEvent) => {
    ...
    setNewBill({ description: '', amount: '', category: 'Utilities', isSubscription: false, contractEndDate: '' });
  };
          ...
          <section className="space-y-6">
            ...
            <div className="space-y-4">
              {profile.expenses.filter(e => e.isRecurring && !e.isSubscription).map(bill => {
                ...
                const daysLeft = daysUntil(isEditing ? editValues.contractEndDate : bill.contractEndDate);
                return (
                    ...
                    <div className="flex items-center space-x-6">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          {(isEditing ? editValues.contractEndDate : bill.contractEndDate) && (
                            <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded ${daysLeft && daysLeft < 90 ? 'bg-amber-100 text-amber-600' : 'bg-slate-100 text-slate-500'}`}>
                              Ends: {new Date(isEditing ? editValues.contractEndDate : bill.contractEndDate).toLocaleDateString('en-GB')} {daysLeft ? `(${daysLeft}d)` : ''}
                            </span>
                          )}
                        </div>
                      </div>
